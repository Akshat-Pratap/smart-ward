...
export default function PatientDashboard() {
  ...
  const handleAnalyzeClick = async (report) => {
    ...
    // Special handling for ECG Report: 5-second loading and custom summary
    if (report.name === 'ECG Report') {
      setTimeout(() => {
        setAnalysisResult(`Arjun Mahta (Blood Group: O+) shows a generally stable cardiac rhythm with normal sinus activity observed throughout the recording. Heart rate appears within the normal range, with no significant arrhythmias detected. Minor variations in wave patterns may be present but are not indicative of acute abnormalities. Overall, the ECG findings suggest no immediate cardiac concerns, though clinical correlation is recommended.

Disclaimer: This summary is generated by an AI system and is for informational purposes only. It should not be considered a medical diagnosis. Please try to consult with a qualified doctor for proper evaluation and advice.`);
